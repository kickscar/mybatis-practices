...
### update
 update 구문은 &lt;update&gt; 엘리먼트를 사용해 구문 매핑이 가능하다. parameterType를 사용하는 방법과 매핑 구문을 호출하는 방법은 앞의 insert 매핑과 유사하다. 예제 ex05를 살펴 보자. MyBatis 설정 파일은 앞의 예제들과 동일하니 생략하고 Mapper 설정부터 살펴보자.
...
```
 ...
 테스트 전 테이블 생성 스크립트를 실행하는 어노테이션 @BeforeAll가 붙은 setup() 메소드는 생략했다. 테스트 순서가 중요하기 때문에 어노테이션 @Order를 사용해 테스트 메소드의 순서를 지정했으며 insert와 update 각각의 쿼리가 실행된 후 반환되는 row 개수로 테스트 통과 조건을 설정했다.  


### delete
 delete 구문 매핑도 앞의 insert, update 구문 매핑과 그 방법이 같다. 일부로 객체 대신 값을 파라미터로 전달하는 넘기는 예제로 제시했는데 살펴보자.
\[예제: mybatis/mapper/ex05/guestbook.xml\]
...
```
 앞의 insert, update와 별반 다를 것은 없으나 사실, 엘리먼트 이름도 delete 매핑 구문에서 &lt;update&gt;에 해도 쿼리는 잘 작동한다. 하지만 의미가 중요하니 쿼리와 맞는 엘리먼트를 쓰는 것이 좋다. delete 매핑 구문을 보면, 일반적인 객체와 달리 값 객체는 필드 이름을 매핑하기가 다소 난감하다. 난감해 하지말고 아무 이름이나 지정하면 된다. 하지만 이것도 의미 있는 이름을 쓰는 것이 가독성에 좋을 것이다. 테스트도 앞의 예제와 같은 방식으로 작성했다.
...
```
 테스트 코드는 앞의 예제와 다르거나 어려운 내용은 없다. 앞의 예제들에서 언급하지 않은 내용은 SqlSession 객체의 commit()을 명시 해주지 않으면 자동 커밋이 되지 않기 때문에 다음 테스트에서 성공 조건을 만족시킬 없다. 따라서 update, delete, insert 테스트에서는 명시적으로 commit() 호출을 해주어야 한다.
