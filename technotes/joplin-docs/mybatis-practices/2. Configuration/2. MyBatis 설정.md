...
1.  &lt;environment&gt;
    ...
    \[예제 mybatis/config/TestConfigEx02\]
...
2.  &lt;dataSource&gt;
    ...
    \[예제 mybatis/config/TestConfigEx03\]
...
## mappers
MyBatis는 SQL과 자바 객체와의 매핑에 주력한 ORM 기술이다. 매핑 도구를 Mapper라 부르는데 XML 또는 Interface에 Anotation을 사용하여 매핑이 가능하다. 각각 XML Mpper 또는 Anotation Mapper라 부른다. 매퍼 설정은 2장 3장에서 자세히 다루고 여기서는 자식 엘리멘트 &lt;mappers&gt;의 속성으로 설정된 Mapper 지정 방법을 알아본다.

1.  resource 속성
    가장 일반적인 방식이다. XML Mapper를 사용할 경우, 클래스 패스에 있는 매퍼 파일을 지정한다.
2.  url 속성
    XML Mapper를 사용할 경우, 절대 경로의 파일시스템 내의 매퍼 파일이나 웹 URL에 존재하는 매퍼 파일 설정에 사용한다.
3.  class
    Annotation Mapper를 사용할 경우 매핑된 인터페이스를 지정한다.

 참고로 Annotation Mapper를 설정한 경우, &lt;package&gt;를 사용하여 매퍼 인터페이스가 위치하는 패키지를 지정할 수 있다.
 간단한 예제로 mappers 설정을 직접 확인해 보자. 예제는 인메모리디비(h2)에 book 테이블을 생성하고 매퍼 파일(boom.xml)의 insert SQL문을 실행 시킨다. 그리고 그 결과를 테스트한다. 테스트를 위해 테이블을 생성하는 부분이 추가되었다.

\[예제 mybatis/mapper/ex04/TestBookMapper\]
...
```xml
<select id="findByNo" parameterType="long" resultType="domain.Book">
 select no, name from book where no = #{no }
</select>
```

resultType 속성에 결과를 매핑할 패키지 경로를 포함한 클래스의 이름을 사용한다. 이 예제에서는 domain.Book 이다. &lt;typeAlias&gt;를 사용하면 패키지 경로를 다 써주는 대신 생략이 가능하다.

\[예제 mybatis/mapper/ex06/book.xml\]
